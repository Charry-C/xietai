<template>
  <div class="relative w-full h-full group">
    ...
    <div
      class="absolute left-4 right-4 sm:left-6 sm:right-6 bottom-4 sm:bottom-6 flex items-end justify-between gap-2 sm:gap-4"
    >
      <div class="text-white min-w-0 flex-1">
        <p class="text-xs uppercase tracking-[0.22em] text-brand-gold/90 mb-1 sm:mb-2">
          {{ t('services.liveProcess') }}
        </p>
        <p class="text-lg sm:text-xl md:text-2xl font-serif font-semibold truncate">
          {{ title || t('services.productionLine') }}
        </p>
      </div>
      <button
        type="button"
